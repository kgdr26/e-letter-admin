...
class MainController extends Controller
{
    ...
    function exportallloan()
    {
        ...
        foreach ($arr as $index => $val) {
            ...
            $sheet->setCellValue('E' . ($startRow + $index), $val->nominal_loan);
            $sheet->setCellValue('F' . ($startRow + $index), $val->bulan_loan . ' Bulan');
            $sheet->setCellValue('G' . ($startRow + $index), $val->loan_perbulan);
            // Kolom H: Tahun Bulan Raw Data Pertama
            $list_pembayaran = json_decode($val->list_pembayaran);
            if (!empty($list_pembayaran)) {
                $first_payment_month = $list_pembayaran[0]->bulan; // Ambil bulan pertama
                $sheet->setCellValue('H' . ($startRow + $index), $first_payment_month); // Misal: 'April 2024'
            } else {
                $sheet->setCellValue('H' . ($startRow + $index), '');
            }

            // Kolom I: Tahun Bulan Raw Data Terakhir
            if (!empty($list_pembayaran)) {
                $last_payment_month = end($list_pembayaran)->bulan; // Ambil bulan terakhir
                $sheet->setCellValue('I' . ($startRow + $index), $last_payment_month); // Misal: 'Oktober 2024'
            } else {
                $sheet->setCellValue('I' . ($startRow + $index), '');
            }
            // Kolom J: Tahun Bulan Sebelum
            $currentMonth = date('m');
            $currentYear = date('Y');
            $previous_month_index = $currentMonth - 1; // Mengurangi 1 bulan

            if ($previous_month_index == 0) {
                $previous_month_index = 12; // Jika bulan Januari, set menjadi Desember
                $currentYear -= 1; // Mundur ke tahun sebelumnya
            }

            // Menyusun format sebagai YYYY-MM
            $formatted_date = $currentYear . '-' . str_pad($previous_month_index, 2, '0', STR_PAD_LEFT);

            $sheet->setCellValue('J' . ($startRow + $index), $formatted_date); // Format menjadi YYYY-MM
        }
